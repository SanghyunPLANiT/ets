...
Parameters
----------
msr_enabled              : bool   – master switch
msr_upper_threshold      : float  – Mt CO₂e bank above which withholding starts
msr_lower_threshold      : float  – Mt CO₂e bank below which release starts
msr_withhold_rate        : float  – fraction of auction_offered withheld (0–1)
msr_release_rate         : float  – Mt released per year from pool when below lower threshold
msr_cancel_excess        : bool   – if True, pool above msr_cancel_threshold is permanently cancelled
msr_cancel_threshold     : float  – Mt CO₂e pool level above which excess is cancelled
"""
